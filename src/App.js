...
import { Button, InitiationModal, Nav, Tile } from "./components";
...
const App = () => {
  ...
  const connectWallet = async () => {
    ...
    try {
      provider?.on("InteractionInitiated", logInteraction);
      provider?.on("InteractionReceived", logInteraction);
      provider?.on("InteractionRejected", logInteraction);
  ...
  return (
    <div className="px-5 py-5 md:px-10">
      ...
      {modalOpen && <InitiationModal setModalOpen={setModalOpen} signer={signer} />}
